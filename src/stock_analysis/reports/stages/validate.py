...
def validate(report: StockReport | None = None, html_path: str | None = None) -> tuple[bool, list[str]]:
    ...
    passed = not any(
        i.startswith("[Schema] 缺失") or i.startswith("[HTML] ") or (i.startswith("[Data] ") and "校验通过" not in i)
        for i in all_issues
    )
...
def validate_data_sanity(html_path: str) -> list[str]:
    ...
    if price_match and ticker_match:
        ...
        if html_price is not None:
            ...
            if actual:
                ...
                if diff_pct > 5:
                    issues.append(
                        f"价格异常偏差: HTML={html_price:.2f}, prices.json={actual:.2f}, 偏差={diff_pct:.1f}% (>5%)"
                    )
                else:
    ...
    # 加密资产使用不同指标 (MCap/TVL, Staking 等)
    is_crypto = False
    try:
        is_crypto = ticker in {"ETH", "SOL", "BNB"}
    except NameError:
        pass
    if is_crypto:
        required_indicators = [
            ("F-Score", r"F-Score|Crypto F-Score"),
        ]
    else:
        required_indicators = [
