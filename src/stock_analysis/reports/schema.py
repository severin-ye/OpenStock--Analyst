...
ALL_MODULES: list[ModuleDef] = [
    ...
    ModuleDef(
        ...
        required=True,
        description="6 个关键 KPI 数字",
    ),
    # S1
    ModuleDef(
        module_id="s1_price_changes",
        section="S1",
        label="涨跌比例总览",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
        description="至少 4 行涨跌数据",
    ),
    ModuleDef(
        module_id="s1_core_judgment",
        section="S1",
        label="核心判断段落",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
    ...
    # S2
    ModuleDef(
        module_id="s2_overview",
        section="S2",
        label="公司/资产概览",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
        description="商业模式、核心指标表",
    ),
    # S3
    ModuleDef(
        module_id="s3_price_chart",
        section="S3",
        label="过去一年走势图",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
        description="priceChart, type=line",
    ),
    ModuleDef(
        module_id="s3_analysis",
        section="S3",
        label="走势分析文字",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
    ...
    ModuleDef(
        ...
        required=True,
        description="L1 MVRV + L2 网络强度 + L3 改编F-Score",
    ),
    ModuleDef(
        module_id="s5_f_score",
        section="S5",
        label="Piotroski F-Score (9项)",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK],
        required=True,
    ),
    ModuleDef(
        module_id="s5_crypto_f_score",
        section="S5",
        label="加密改编 F-Score",
        asset_categories=[AssetCategory.CRYPTO],
        required=True,
    ...
    ModuleDef(
        module_id="s5_valuation_radar",
        section="S5",
        label="估值雷达图",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK],
        required=True,
        description="valuationRadar, type=radar",
    ),
    ModuleDef(
        module_id="s5_peer_comparison",
        section="S5",
        label="同行估值对比图",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK],
        required=True,
        description="peerCompareChart, type=bar",
    ),
    ModuleDef(
        module_id="s5_crypto_metrics_chart",
        section="S5",
        label="加密关键指标图",
        asset_categories=[AssetCategory.CRYPTO],
        required=True,
        description="vmScoreChart + mvrvChart",
    ),
    ModuleDef(
        module_id="s5_dcf_chart",
        section="S5",
        label="DCF/目标价对比图",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
        description="dcfChart, type=bar",
    ),
    ModuleDef(
        module_id="s5_revenue_timeline",
        section="S5",
        label="季度营收趋势图",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK],
        required=False,
        description="timelineChart, type=bar, 可选",
    ),
    ModuleDef(
        module_id="s5_dashboard",
        section="S5",
        label="关键指标仪表盘",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
    ...
    # S6
    ModuleDef(
        module_id="s6_scenario_chart",
        section="S6",
        label="情景分析图",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
        description="scenarioChart, type=bar/line",
    ),
    ModuleDef(
        module_id="s6_scenarios",
        section="S6",
        label="情景分析文字",
        asset_categories=[AssetCategory.STOCK, AssetCategory.HK_STOCK, AssetCategory.CRYPTO],
        required=True,
        description="至少 3 个情景",
    ),
    # S7
    ModuleDef(
        module_id="s7_risks",
...
class ModuleOverride(BaseModel):
    ...
    alternative_modules: list[str] = Field(default_factory=list)  # 替代模块 ID 列表
    search_attempts: list[str] = Field(default_factory=list)  # 已尝试的搜索策略
    round: int = 1  # 第几轮协商
...
class StockReport(BaseModel):
    ...
    cover_kpi: list[KPIItem] = Field(default_factory=list)
    cover_title: str = ""
    cover_price: str = ""
    cover_market_cap: str = ""

    s1_price_changes: list[PriceChangeRow] = Field(default_factory=list)
    ...
    greenblatt_ranking: list[RankingRow] = Field(default_factory=list)
    ranking_summary: str = ""
    f_score_items: list[FScoreItem] = Field(default_factory=list)
    f_score_total: str = "0"
    ...
    s5_body_html: str = ""
    s5_valuation_methods: list[ValuationMethod] = Field(default_factory=list)

    dashboard_metrics: list[KeyMetricRow] = Field(default_factory=list)

    s6_body_html: str = ""
    s6_scenarios: list[ScenarioRow] = Field(default_factory=list)

    s7_risks: list[RiskItem] = Field(default_factory=list)
    ...
    overrides: list[ModuleOverride] = Field(default_factory=list)
