...
def _equity_layer_scores(info: PriceSnapshot) -> dict[str, float]:
    return {
        ...
        "L4": NEUTRAL_SCORE_10
        if (info.peg_num is not None and info.peg_num < 0)
        else _score_from_anchors(
            info.peg_num,
            [
                (0.0, 10.0),
                (0.5, 9.0),
...
def _btc_layer_scores(info: PriceSnapshot) -> dict[str, float]:
    return {
        ...
        "L3": _score_linear(info.f_score, 9),
        "L4": _score_from_anchors(
            info.days_since_halving,
            [
                (0.0, 10.0),
                (180.0, 9.0),
                (360.0, 8.0),
...
def compute_greenblatt(
    ...
    max_possible = len(all_ebit_ev) * 0.40 + len(all_roic) * 0.25 + len(all_f_score) * 0.25 + len(all_peg) * 0.10
...
def compute_crypto_ranking(
    ...
    l4_rank = rank_lower({k: float(v) for k, v in all_halving_days.items()}, "BTC")
    ...
    summary = f"加权综合 = {l1r}×40% + {l2r}×25% + {l3r}×25% + {l4r}×10% = {composite_score:.2f}"

    max_possible = (
        len(all_mvrv) * 0.40 + len(all_hash_rate) * 0.25 + len(all_f_score) * 0.25 + len(all_halving_days) * 0.10
    )
    score_10 = composite_to_score10(composite_score, max_possible)
...
def compute_pos_crypto_ranking(
    ...
    max_possible = (
        len(all_mcap_tvl) * 0.40 + len(all_staking) * 0.25 + len(all_f_score) * 0.25 + len(all_inflation) * 0.10
    )
    score_10 = composite_to_score10(composite_score, max_possible)
