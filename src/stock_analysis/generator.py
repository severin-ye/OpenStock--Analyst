...
.rank-card{background:var(--white);
...
th{
  ...
  letter-spacing:.4px;color:var(--slate);border-bottom:1px solid var(--border);white-space:nowrap}
...
.method .formula{font-family:'SF Mono','Fira Code',monospace;
  font-size:12px;
  color:var(--slate);
  background:#f1f5f9;
  padding:8px 14px;
...
def _card_html(
    ...
    for label, value, pos in metrics:
        lines.append(
            f'      <div class="m"><label>{label}</label><span>{value}</span><span class="pos">{pos}</span></div>'
        )
    lines.append("    </div>")
    lines.append("  </div></a>")
    return "\n".join(lines)
...
def _build_market_section(label: str, tickers: list[str], all_rankings: list[tuple]) -> str:
    ...
    chart_id = (
        f"chart_{label.replace(' ', '').replace('🇺🇸', 'us')}"
        f"{{label.replace('🇭🇰', 'hk').replace('₿', 'crypto')}}"
    )
  ...
  data:{{
    ...
    datasets:[{{
      ...
      backgroundColor:{json.dumps(chart_colors_local)},
      borderRadius:6,
      borderSkipped:false
    }}]
  }},
